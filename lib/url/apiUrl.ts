...
export const dojangUrl = (date: string, ocid?: string) =>
    `${baseUrl}/maplestory/v1/ranking/dojang?date=${date}&difficulty=1${ocid ? `&ocid=${ocid}` : ""}`; // 무릉도장 (ocid를 주면 해당 캐릭터만 조회)
export const theseedUrl = (date: string) => `${baseUrl}/maplestory/v1/ranking/theseed?date=${date}`; // 더시드
export const achievementUrl = (date: string, ocid?: string) =>
    `${baseUrl}/maplestory/v1/ranking/achievement?date=${date}${ocid ? `&ocid=${ocid}` : ""}`; // 업적 (ocid를 주면 해당 캐릭터만 조회)
