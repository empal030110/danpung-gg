...
export default function ItemBox({
        ...
        return (
            <div className="border border-neutral-400 p-[12px] rounded-[8px] mb-[8px]">
                <div className="flex justify-start items-center gap-[8px] h-full">
                    <Image
                        src={filterItem?.android_icon || ""}
                        alt={filterItem?.android_name || ""}
                        width={30}
                        height={30}
                        loading="lazy"
                    />
        ...
        return (
            <>
                <button
                    ...
                    className="w-full text-left border border-neutral-400 p-[12px] rounded-[8px] mb-[8px] cursor-pointer hover:bg-gray-100 dark:hover:bg-neutral-800"
                >
                    <div className="flex justify-start items-center gap-[8px] h-full">
                        <Image
                            src={filterItem?.title_icon || ""}
                            alt={filterItem?.title_name || ""}
                            width={30}
                            height={30}
                            loading="lazy"
                        />
                ...
                {showDetail && <TitleDetailModal title={filterItem} onClose={() => setShowDetail(false)} />}
            </>
        );
    }
    ...
    return (
        <>
            ...
            >
                <div
                    className={`flex justify-start items-center gap-[8px] ${filterItem.potential_option_grade ? "mb-[8px]" : "h-full"}`}
                >
                    <Image
                        src={filterItem?.item_icon || ""}
                        alt={filterItem?.item_name || ""}
                        width={30}
                        height={30}
                    ...
                    <div className="flex flex-col gap-[2px]">
                        {Number(filterItem?.starforce) > 0 && (
                            <div className="flex gap-[2px] items-center text-[12px]">
                                <FaStar size={12} color="oklch(85.2% 0.199 91.936)" />x{filterItem?.starforce}
                            </div>
                        )}
                ...
                {filterItem.potential_option_grade && (
                    <div className="flex flex-wrap text-[12px] gap-[4px]">
                        <p>잠재</p>
                        {filterItem.potential_option_1 && (
                            <p className={gradeColor(filterItem.potential_option_grade)}>
                                {cutOptionName(filterItem.potential_option_1)}
                            </p>
                        )}
                        {filterItem.potential_option_2 && (
                            <p className={gradeColor(filterItem.potential_option_grade)}>
                                {cutOptionName(filterItem.potential_option_2)}
                            </p>
                        )}
                        {filterItem.potential_option_3 && (
                            <p className={gradeColor(filterItem.potential_option_grade)}>
                                {cutOptionName(filterItem.potential_option_3)}
                            </p>
                        )}
                    </div>
                )}
                {filterItem.additional_potential_option_grade && (
                    <div className="flex flex-wrap text-[12px] gap-[4px] mt-[4px]">
                        ...
                        {filterItem.additional_potential_option_1 && (
                            <p className={gradeColor(filterItem.additional_potential_option_grade)}>
                                {cutOptionName(filterItem.additional_potential_option_1)}
                            </p>
                        )}
                        {filterItem.additional_potential_option_2 && (
                            <p className={gradeColor(filterItem.additional_potential_option_grade)}>
                                {cutOptionName(filterItem.additional_potential_option_2)}
                            </p>
                        )}
                        {filterItem.additional_potential_option_3 && (
                            <p className={gradeColor(filterItem.additional_potential_option_grade)}>
                                {cutOptionName(filterItem.additional_potential_option_3)}
                            </p>
                        )}
                    </div>
                )}
            </button>
            {showDetail && <ItemDetailModal item={filterItem} onClose={() => setShowDetail(false)} />}
        </>
    );
}
